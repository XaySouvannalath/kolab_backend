...
async def get_review_image(review_image_id: int):
    query = "SELECT * FROM review_image WHERE id = :id"
    return await database.fetch_one(query=query, values={"id": review_image_id})

async def create_review_image(review_image: ReviewImage):
    query = """
    INSERT INTO review_image (review_id, image_name, created_by)
    VALUES (:review_id, :image_name, :created_by)
    """
    values = {
        "review_id": review_image.review_id,
        "image_name": review_image.image_name,
...
async def update_review_image(review_image_id: int, review_image: ReviewImage):
    query = """
    UPDATE review_image
    SET review_id = :review_id, image_name = :image_name, created_by = :created_by,
